...
for (file in file_list_2023) {
  ...
  count_ExMan <- sum(filtered_data_2023$prediction == "ExFlap", filtered_data_2023$prediction == "Manouvre")
  
  results_2023 <- rbind(results_2023, data.frame(
    file_name = basename(file),
    count_ExMan = count_ExMan,
    stringsAsFactors = FALSE
  ))
    }
...
for (file in file_list_2024) {
  ...
  count_ExMan <- sum(filtered_data_2024$prediction == "ExFlap", filtered_data_2024$prediction == "Manouvre")
...
model <- glmer(count_ExMan ~ inPark + (1 | device_info_serial) + (1 | year), data = files_ExMan, family = poisson)
summary(model)
...
count_ExMan <- ggplot(files_ExMan, aes(x = inPark, y = count_ExMan, fill = inPark)) +
     geom_boxplot() +
  ...
  labs(
    x = "Year",
    y = "count of ExFlap or Manouvre",
    fill = "Wind Park"
  )
ggsave(
   filename =  "count_ExMan.pdf",
   plot = count_ExMan,
    scale = 1,
    width = 5,
    height = 5
...
summary_stats_ExMan <- files_ExMan %>%
  group_by(year, inPark) %>%
  summarise(
    mean_proportion = mean(count_ExMan),
    min_proportion = min(count_ExMan),
    max_proportion = max(count_ExMan),  # Fixed column name
    .groups = 'drop'
  )
...
tracker_data <- all_tracker_data_with_inPark_2023%>%
  filter(file_name=="5579_2023-07-17 02:06:31_2023-07-17 05:13:04.csv")
